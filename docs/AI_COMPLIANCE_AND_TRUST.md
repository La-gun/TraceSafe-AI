...
|--------|-------------------|-------------------|
| Inspector console | `server/functions/inspectorAI/entry.ts` | Plans **server-side tool calls**, answers from **retrieved rows only**, optional **consistency review** (“judge”) pass/revise. |
| Consumer assist (demo / assist flow) | `server/functions/consumerAssist/entry.ts` | **Vision**: reads a product label image to suggest a serial format (invokes model with image URL). |
| Field scan advisory | `server/functions/scanTag/entry.ts` | **Risk scoring text** is explicitly **advisory** and does not replace rule-based scan status (see in-code disclaimer). |

LLM calls are made via the hosted backend’s `integrations.Core.InvokeLLM` from server functions. **Subprocessors and retention** for model inference follow **your hosting agreement** and **your model provider’s terms** — confirm current policies in vendor docs for questionnaires.
...
2. **Prompt-injection hardening** — User and history text is **length-limited** and **delimiter-sanitized** (`server/functions/inspectorAI/promptGuards.ts`) so chat content cannot easily spoof system boundaries.
...
Organisations should document: **who may use AI features**, **whether prompts/logs are allowed to contain personal data**, and **host + model provider** as subprocessors where applicable.
...
- Your **hosted backend** runs function execution and **InvokeLLM**; confirm **regions, subprocessors, and log retention** with that provider for your deployment.
