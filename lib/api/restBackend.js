...
async function request(method, path, { json, formData } = {}) {
  ...
  if (isHttpLoggingEnabled()) {
    // Intentionally log before fetch() so requestId is always visible.
    // Avoid logging FormData bodies (can contain files).
    // eslint-disable-next-line no-console
    console.log('[http]', { requestId, method, url, body: formData ? '[FormData]' : json });
  }
  ...
  if (!res.ok) {
    ...
    throw new ApiError(data.message || `HTTP ${res.status}`, { status: res.status, data, requestId });
  }
  if (isHttpLoggingEnabled()) {
    // eslint-disable-next-line no-console
    console.log('[http:ok]', {
    ...
    integrations: {
      Core: {
        async UploadFile({ file }) {
          ...
          if (isHttpLoggingEnabled()) {
            // eslint-disable-next-line no-console
            console.log('[http]', { requestId, method: 'POST', url, body: '[FormData:file]' });
          }
          ...
          if (!res.ok) {
            ...
            throw new ApiError(data.message || `Upload failed: ${res.status}`, { status: res.status, data, requestId });
          }
          if (isHttpLoggingEnabled()) {
            // eslint-disable-next-line no-console
            console.log('[http:ok]', {
