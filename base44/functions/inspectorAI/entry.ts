  ...
  switch (name) {
    case 'snapshot_workspace': {
      const [reports, alerts, batches, scans] = await Promise.all([
        base44.asServiceRole.entities.ConsumerReport.list('-created_date', 15),
        base44.asServiceRole.entities.DiversionAlert.list('-created_date', 15),
        base44.asServiceRole.entities.Batch.list('-created_date', 15),
        base44.asServiceRole.entities.ScanEvent.list('-created_date', 25),
      ]);
      return [
    ...
    case 'list_consumer_reports': {
      ...
      const rows = await base44.asServiceRole.entities.ConsumerReport.list('-created_date', 50);
    ...
    case 'list_diversion_alerts': {
      ...
      const rows = await base44.asServiceRole.entities.DiversionAlert.list('-created_date', 50);
    ...
    case 'list_batches': {
      const limit = clampInt(args.limit, 30, 1, 50);
      const rows = await base44.asServiceRole.entities.Batch.list('-created_date', 50);
    ...
    case 'list_scan_events': {
      ...
      const rows = await base44.asServiceRole.entities.ScanEvent.list('-created_date', 80);
    ...
    case 'get_batch_by_number': {
      const bn = String(args.batch_number ?? '').trim();
      if (!bn) return [];
      const rows = await base44.asServiceRole.entities.Batch.filter({ batch_number: bn });
...
${conversationHistory ? `Conversation so far:\n${conversationHistory}\n\n` : ''}Latest turn:\n${wrappedQuestion}`;
    ...
    for (const call of toolCalls) {
      const rows = await runTool(base44, call.tool, call.arguments);
...
${conversationHistory ? `Conversation so far:\n${conversationHistory}\n\n` : ''}Latest turn:\n${wrappedQuestion}`;
    ...
    const rawAnswer = await base44.asServiceRole.integrations.Core.InvokeLLM({
      prompt: answerPrompt,
