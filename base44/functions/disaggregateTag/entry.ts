...
Deno.serve(async (req) => {
  try {
    ...
    const links = await base44.asServiceRole.entities.AggregationLink.filter({
      child_tag_uid,
      status: 'active',
    });
    ...
    await base44.asServiceRole.entities.AggregationLink.update(link.id, {
      status: 'disaggregated',
      disaggregated_at: new Date().toISOString(),
    });

    // Clear parent on the tag registry
    const tags = await base44.asServiceRole.entities.TagRegistry.filter({ tag_uid: child_tag_uid });
    if (tags[0]) {
