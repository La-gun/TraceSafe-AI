  ...
  try {
    ...
    if (fullLifecycleDemo) {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        try {
          const existing = await base44.asServiceRole.entities.Partner.filter({ name });
          if (existing[0]) {
          ...
          try {
            const p = await base44.asServiceRole.entities.Partner.create({
              name,
            ...
            const p = await base44.asServiceRole.entities.Partner.create({ name });
            partnerIds.push(p.id);
            summary.partners_created++;
          }
        } catch (e) {
      ...
      for (let i = 1; i <= EXTENDED_TAG_COUNT; i++) {
        ...
        try {
          const ex = await base44.asServiceRole.entities.TagRegistry.filter({ tag_uid: uid });
          if (ex[0]) {
            summary.extended_tags_existing++;
            continue;
          }
          const bid = batchIdByNumber[bn] ||
            (await base44.asServiceRole.entities.Batch.filter({ batch_number: bn }))[0]?.id;
          const today = new Date().toISOString().slice(0, 10);
          await base44.asServiceRole.entities.TagRegistry.create({
            tag_uid: uid,
            tag_type: "item",
      ...
      try {
        const exCase = await base44.asServiceRole.entities.TagRegistry.filter({ tag_uid: CASE_TAG_UID });
        if (!exCase[0]) {
          const today = new Date().toISOString().slice(0, 10);
          await base44.asServiceRole.entities.TagRegistry.create({
        ...
        for (let c = 1; c <= 3; c++) {
          ...
          const ex = await base44.asServiceRole.entities.TagRegistry.filter({ tag_uid: childUid });
          if (ex[0]) {
            summary.extended_tags_existing++;
            continue;
          }
          ...
          const linkEx = await base44.asServiceRole.entities.AggregationLink.filter({
            parent_tag_uid: CASE_TAG_UID,
      ...
      try {
        const recent = await base44.asServiceRole.entities.ScanEvent.list("-created_date", 800);
        ...
        for (let n = 0; n < toAdd; n++) {
          ...
          const tags = await base44.asServiceRole.entities.TagRegistry.filter({ tag_uid: uid });
          const tag = tags[0];
          if (!tag) continue;
      ...
      try {
        const allAlerts = await base44.asServiceRole.entities.DiversionAlert.list("-created_date", 400);
        const have = new Set(
        ...
        for (let i = 1; i <= 24; i++) {
          ...
          const batches = await base44.asServiceRole.entities.Batch.filter({ batch_number: b.batch_number });
          const batch = batches[0];
          await base44.asServiceRole.entities.DiversionAlert.create({
            alert_number: alertNum,
      ...
      try {
        const crList = await base44.asServiceRole.entities.ConsumerReport.list("-created_date", 200);
        const haveS = new Set(
      ...
      try {
        const irList = await base44.asServiceRole.entities.InspectionReport.list("-created_date", 200);
        const haveR = new Set(
      ...
      for (let i = 0; i < 14; i++) {
        ...
        try {
          const ex = await base44.asServiceRole.entities.BatchStatus.filter({ batch_number: b.batch_number });
          if (ex[0]) {
          ...
          await base44.asServiceRole.entities.BatchStatus.create({
            batch_number: b.batch_number,
            product_name: b.product_name,
            manufacturer: b.manufacturer_name,
            status: st,
      ...
      try {
        const transfers = await base44.asServiceRole.entities.CustodyTransfer.list("-created_date", 200);
        ...
        for (let i = 1; i <= 18; i++) {
          ...
          await base44.asServiceRole.entities.CustodyTransfer.create({
