...
Deno.serve(async (req) => {
  try {
    ...
    const transfer = await api.asServiceRole.entities.CustodyTransfer.create({
    ...
    if (from_partner_id) {
      const partners = await api.asServiceRole.entities.Partner.filter({ id: from_partner_id });
      if (partners[0]) {
        await api.asServiceRole.entities.Partner.update(partners[0].id, {
