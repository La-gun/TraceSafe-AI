...
Deno.serve(async (req) => {
  try {
    ...
    const existing = await api.asServiceRole.entities.TagRegistry.filter({ tag_uid });
    ...
    const tag = await api.asServiceRole.entities.TagRegistry.create({
      tag_uid,
      tag_type,
      product_id,
      batch_id,
    ...
    if (parent_tag_uid) {
      const parentTags = await api.asServiceRole.entities.TagRegistry.filter({ tag_uid: parent_tag_uid });
    ...
    if (batch_id) {
      const batches = await api.asServiceRole.entities.Batch.filter({ id: batch_id });
      if (batches[0]) {
        await api.asServiceRole.entities.Batch.update(batch_id, {
