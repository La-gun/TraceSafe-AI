...
Deno.serve(async (req) => {
  try {
    ...
    const existing = await api.asServiceRole.entities.BatchStatus.filter({ batch_number });
    let batchStatusRecord;
    if (existing.length > 0) {
      batchStatusRecord = await api.asServiceRole.entities.BatchStatus.update(existing[0].id, {
        status,
        reason,
      ...
      const batches = await api.asServiceRole.entities.Batch.filter({ batch_number });
      const batch = batches[0];
      batchStatusRecord = await api.asServiceRole.entities.BatchStatus.create({
        batch_number,
    ...
    const batches = await api.asServiceRole.entities.Batch.filter({ batch_number });
    if (batches.length > 0) {
      await api.asServiceRole.entities.Batch.update(batches[0].id, {
