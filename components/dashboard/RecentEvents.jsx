...
const eventTypeConfig = {
  ...
  returns: { icon: Warehouse, label: "Returns", color: "text-orange-400 bg-orange-500/10" },
  seizure: { icon: Ship, label: "Seizure", color: "text-red-400 bg-red-500/10" },
  suspicious_scan: { icon: User, label: "Suspicious Scan", color: "text-rose-400 bg-rose-500/10" },
};
...
export default function RecentEvents({ recentEvents }) {
  const events = useMemo(() => {
    if (recentEvents?.length) {
      return recentEvents.map((e, idx) => {
        let time = "—";
        try {
          if (e.created_date) {
            time = formatDistanceToNow(new Date(e.created_date), { addSuffix: true });
          }
        } catch {
          time = "—";
        }
        return {
          key: e.id || `ev-${idx}`,
          type: e.event_type || "commissioning",
  ...
  const source = recentEvents?.length ? "Scan ledger (API)" : "Illustrative";
    ...
    <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Supply chain touchpoints — {source}</p>
        </div>
      </div>
      <div className="space-y-2.5">
        {events.map((event, i) => {
          const config = eventTypeConfig[event.type] || eventTypeConfig.commissioning;
          return (
            <div
              key={event.key ?? i}
