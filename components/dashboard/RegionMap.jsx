    ...
    <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Scan distribution by state</p>
        </div>
      </div>
      <div className="space-y-2.5">
        {regions.map((region, i) => {
          const pct = (region.scans / 8420) * 100;
          const susPct = region.suspicious > 20 ? "high" : region.suspicious > 10 ? "medium" : "low";
          return (
            <div key={i} className="flex items-center gap-3">
              <span className="text-xs text-gray-400 w-24 shrink-0">{region.name}</span>
